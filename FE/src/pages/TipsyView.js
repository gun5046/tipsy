...
import tipsyConfig from '../phaser/tipsyConfig';
...
const TipsyView = () => {
  ...
  // 새로 추가!!!!!!!!!!!!!!!!!11
  const socket = new SockJS('http://domain-url/ws/chat')
  ...
  // 건물번호 1,2,3
  const url = 'http://domain-url/room'
  ...
  useEffect(() => {
    ...
    if (!startGame && phaserEl.current) {
      const newGame = new phaser.Game(tipsyConfig)
  ...
  }, [tipsyConfig, phaserEl, startGame])
